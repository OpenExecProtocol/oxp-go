...
params := FooParams{
	...
	Description: oxp.Null[string](),

	Point: oxp.F(oxp.Point{
		X: oxp.Int(0),
		Y: oxp.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: oxp.Raw[int64](0.01), // sends a float
	}),
}
```
...
client.Tools.List(context.TODO(), ...,
	// Override the header
...
`*oxp.Error`. This contains the `StatusCode`, `*http.Request`, and
...
if err != nil {
	...
	panic(err.Error()) // GET "/tools": 400 Bad Request { ... }
}
```
...
We also provide a helper `oxp.FileParam(reader io.Reader, filename string, contentType string)`
...
params := FooNewParams{
    ID:   oxp.F("id_xxxx"),
    Data: oxp.F(FooNewParamsData{
        FirstName: oxp.F("John"),
    }),
}
